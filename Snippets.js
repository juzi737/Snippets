...
let yourUUID = '73fe2d39-6415-4a09-9c8e-72d093f1971f';  // UUID

// CDN
let cfip = [
    'mfa.gov.ua', 'saas.sin.fan', 'store.ubi.com','cf.130519.xyz','cf.008500.xyz','openai.com','stackexchange.com','jobsdb.com',
    ...
    async fetch(request, env, ctx) {
        try {
            ...
            if (request.headers.get('Upgrade') === 'websocket') {
                const proxyipB64 = url.searchParams.get('proxyip_b64');
                let customProxyIP =
                  url.searchParams.get('proxyip') ||
                  request.headers.get('X-ProxyIP') ||
                  request.headers.get('x-proxyip');
              
                //新增：从路径中解析 Base64（兼容 Shadowrocket 直接写 /xxxx）
                if (!customProxyIP) {
                  const pathSegment = url.pathname.replace('/', '').trim();
                  if (pathSegment && pathSegment.length > 6) {
                    try {
                      const decoded = atob(pathSegment);
                      if (
                        decoded.includes('.') ||
                        decoded.includes(':') ||
                        decoded.includes('socks') ||
                        decoded.includes('http')
                      ) {
                        customProxyIP = decoded;
                      }
                    } catch (e) {
                      // 非 Base64 内容忽略
                    }
                ...
                //旧逻辑保留（支持 proxyip_b64 参数）
                if (!customProxyIP && proxyipB64) {
                  try {
                    customProxyIP = atob(proxyipB64);
                  } catch (e) {
                    // 解码错误忽略
                  }
                }
